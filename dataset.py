...
class Dataset:
    ...
    def get_datas(self, days: int, tickers: List[str], ts: str) -> pd.DataFrame:
        datas = [self._download_data(days=days, ticker=i, ts=ts) for i in tickers]
    ...
    def _download_data(self, days: int = 10, ticker: str = 'BTCUSDT', ts: str = '1h') -> pd.DataFrame:
        if ts not in self.time_frames:
            raise ValueError(f'Supported time frames: {self.time_frames}')
